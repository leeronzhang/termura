...
enum RuleFileParser {
    static func parse(_ content: String) -> [RuleSection] {
        ...
        for (index, line) in lines.enumerated() {
            ...
            if let (heading, level) = parseHeading(line) {
                if let prevHeading = currentHeading {
                    ...
                    sections.append(RuleSection(heading: prevHeading, level: currentLevel, body: body, lineRange: range))
                }
        ...
        if let heading = currentHeading {
            ...
            sections.append(RuleSection(heading: heading, level: currentLevel, body: body, lineRange: range))
        }
