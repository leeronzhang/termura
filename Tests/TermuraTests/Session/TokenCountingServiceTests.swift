...
struct TokenCountingServiceTests {
    ...
    func fourHundredCharsIsHundredTokens() async {
        ...
        await service.accumulateOutput(for: sessionID, text: text)
    ...
    func accumulationsAreAdditive() async {
        ...
        await service.accumulateOutput(for: sessionID, text: String(repeating: "x", count: 200))
        await service.accumulateOutput(for: sessionID, text: String(repeating: "y", count: 200))
        let tokens = await service.estimatedTokens(for: sessionID)
    ...
    func resetClearsCount() async {
        let service = TokenCountingService()
        let sessionID = SessionID()
        await service.accumulateOutput(for: sessionID, text: String(repeating: "z", count: 400))
    ...
    func multiSessionIsolation() async {
        ...
        await service.accumulateOutput(for: sessionA, text: String(repeating: "a", count: 400))
        await service.accumulateOutput(for: sessionB, text: String(repeating: "b", count: 800))
    ...
    func resetIsolation() async {
        ...
        await service.accumulateOutput(for: sessionA, text: String(repeating: "a", count: 400))
        await service.accumulateOutput(for: sessionB, text: String(repeating: "b", count: 400))
