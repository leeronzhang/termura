...
struct PromptDetectionTests {
    ...
    private func makeViewModel(
        ...
        let coordinator = AgentCoordinator(sessionID: sessionID, sessionStore: MockSessionStore(), agentStateStore: MockAgentStateStore())
    ...
    @MainActor func detectAIPromptSetsFlag() async {
        let (vm, _, modeCtrl) = makeViewModel(lines: ["> "])
        modeCtrl.switchToPassthrough()
        await vm.detectPromptFromScreenBuffer()
    ...
    @MainActor func detectShellPromptSetsFlag() async {
    ...
    @MainActor func detectEmptyBufferNoSwitch() async {
        let (vm, _, modeCtrl) = makeViewModel(lines: [])
        modeCtrl.switchToPassthrough()
        await vm.detectPromptFromScreenBuffer()
        #expect(modeCtrl.mode == .passthrough)
    ...
    @MainActor func detectNonPromptNoSwitch() async {
