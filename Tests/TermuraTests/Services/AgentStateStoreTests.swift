...
struct AgentStateStoreTests {
    ...
    @Test("remove clears existing sidebar row state")
    func removeClearsSidebarRowState() {
        let store = makeStore()
        let sid = SessionID()
        let rowState = store.sidebarRowState(for: sid)

        store.update(state: makeAgent(sessionID: sid, status: .error, tokenCount: 2048))
        #expect(rowState.status == .error)
        #expect(rowState.tokenSummary != nil)

        store.remove(sessionID: sid)

        #expect(rowState.status == nil)
        #expect(rowState.agentType == nil)
        #expect(rowState.tokenSummary == nil)
        #expect(rowState.durationText == nil)
        #expect(rowState.currentTaskSnippet == nil)
    }

    @Test("remove keeps sidebar row state reusable for later updates")
    func removeKeepsSidebarRowStateReusable() {
        let store = makeStore()
        let sid = SessionID()
        let rowState = store.sidebarRowState(for: sid)

        store.update(state: makeAgent(sessionID: sid, status: .error))
        store.remove(sessionID: sid)
        store.update(state: makeAgent(sessionID: sid, status: .thinking))

        #expect(store.sidebarRowState(for: sid) === rowState)
        #expect(rowState.status == .thinking)
    }
