...
final class ExperienceCodifierTests: XCTestCase {
    // XCTestCase populates these in setUp(); IUO is the standard fixture
    // shape because XCTestCase has no throwing init.
    ...
    func testAppendRuleWritesToFile() async throws {
        ...
        // Cleanup; swallow error because the assertion already succeeded.
    ...
    func testAppendRuleToNonexistentFileThrows() async {
        ...
        do {
            ...
            XCTAssertTrue(true, "thrown as expected: \(error.localizedDescription)")
        }
    }
}
